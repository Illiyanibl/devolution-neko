<template>
  <div id="neko" :class="[expanded ? 'expanded' : '', embedMode ? 'embed' : '', castMode ? 'cast' : '']">
    <main class="neko-main">
      ...
      <!-- Devolution: floating клавиатура-кнопка для embed/cast режимов.
           DOM-button = user-gesture внутри WKWebView, focus на
           textarea.neko-overlay → iOS поднимает экранную клавиатуру.
           Дублирует встроенную Neko-кнопку, но не зависит от .room-container,
           который в cast-режиме скрыт. -->
      <button
        v-if="loaded && isTouchDevice && (embedMode || castMode)"
        class="dvl-kbd-btn"
...
<style lang="scss">
  ...
  /* Embed-режим (используется внешними приложениями типа Devolution):
     показываем только видеоканвас, без header/sidebar/chat. */
  #neko.embed {
    .header-container { display: none; }
    .neko-menu { display: none; }
    main.neko-main { width: 100%; }
  }
  /* Cast-режим — добавочно скрывает нижнюю room-панель. */
  #neko.cast {
    .room-container { display: none; }
  }

  /* Floating клавиатура-кнопка для embed/cast режимов. */
  .dvl-kbd-btn {
    position: fixed;
    bottom: 14px;
    right: 14px;
...
<script lang="ts">
  ...
  export default class extends Vue {
    ...
    /// Embed-режим (`?embed=1`) — скрываем header, sidebar, chat. Внешний
    /// мобильный клиент использует только канвас.
    get embedMode(): boolean {
      return new URL(location.href).searchParams.get('embed') === '1'
    }

    /// Cast-режим (`?cast=1`) — дополнительно скрывает нижнюю room-панель
    /// (controls, cursor toggle). В сочетании с embed остаётся только канвас.
    get castMode(): boolean {
      return new URL(location.href).searchParams.get('cast') === '1'
    }
