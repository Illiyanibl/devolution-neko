...
<script lang="ts">
  ...
  export default class extends Vue {
    ...
    mounted() {
      ...
      // Auto-login через query params: ?usr=...&pwd=...
      // Используется внешним мобильным клиентом для прозрачного входа.
      const params = new URL(location.href).searchParams
      const qUsr = params.get('usr')
      const qPwd = params.get('pwd')
      if (qUsr && qPwd) {
        this.username = qUsr
        this.password = qPwd
        // Удаляем креды из URL чтобы не светились в history.
        const cleaned = new URL(location.href)
        cleaned.searchParams.delete('usr')
        cleaned.searchParams.delete('pwd')
        history.replaceState({}, '', cleaned.toString())

        // Логинимся, затем сразу connect — иначе пользователю пришлось бы жать кнопку.
        ;(async () => {
          try {
            await this.neko.login(this.username, this.password)
            await this.neko.connect()
